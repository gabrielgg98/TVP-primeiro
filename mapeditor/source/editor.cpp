...
bool Editor::importMap(FileName filename, int import_x_offset, int import_y_offset, int import_z_offset, ImportType house_import_type, ImportType spawn_import_type)
{
	...
	std::map<Position, Creature*> creature_map;
	if(spawn_import_type != IMPORT_DONT) {
		for (SpawnPositionList::iterator siter = imported_map.tvpSpawns.begin(); siter != imported_map.tvpSpawns.end();) {
			...
			case IMPORT_MERGE: {
				...
				if (imported_tile) {
					ASSERT(imported_tile->creature);
					creature_map[new_spawn_pos] = imported_tile->creature;
					...
					++next;
					if (next == imported_map.tvpSpawns.end())
						cont = false;
					else
						next_spawn = *next;
					imported_map.tvpSpawns.erase(siter);
					if (cont)
						siter = imported_map.tvpSpawns.find(next_spawn);
					else
						siter = imported_map.tvpSpawns.end();
				}
				break;
			}
			case IMPORT_DONT: {
	...
	for(MapIterator mit = imported_map.begin(); mit != imported_map.end(); ++mit) {
		...
		if(old_tile) {
			...
			map.tvpSpawns.removeSpawn(old_tile);
		}
	...
	for (std::map<Position, Creature*>::iterator spawn_iter = creature_map.begin(); spawn_iter != creature_map.end(); ++spawn_iter) {
		Position pos = spawn_iter->first;
		TileLocation* location = map.createTileL(pos);
		Tile* tile = location->get();
		if (!tile) {
			...
			map.tvpSpawns.removeSpawn(tile);
		}
		tile->creature = spawn_iter->second;

		map.tvpSpawns.addSpawn(tile);
	}
