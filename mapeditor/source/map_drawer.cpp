...
void MapDrawer::BlitItem(int& draw_x, int& draw_y, const Tile* tile, const Item* item, bool ephemeral, int red, int green, int blue, int alpha) {
	...
	if(it.isSplash() || it.isFluidContainer()) {
		subtype = getLiquidColor(item->getSubtype());
...
void MapDrawer::WriteTooltip(Item* item, std::ostringstream& stream)
{
	...
	const Teleport* const teleport = dynamic_cast<const Teleport*>(item);
	const Door* const door = dynamic_cast<const Door*>(item);
	const Key* const key = dynamic_cast<const Key*>(item);

	if (unique == 0 && action == 0 && text.empty() && !teleport && !door && !key) {
		return;
	}
	...
	if (door && door->getKeyHoleNumber() == 0 && door->getDoorLevel() == 0 && door->getQuestNumber() == 0 && door->getQuestValue() == 0) {
		return;
	}

	if (key && key->getKeyNumber() == 0) {
		return;
	}
	...
	if (door) {
		const uint16_t keyHoleNumber = door->getKeyHoleNumber();
		if (keyHoleNumber != 0) {
			stream << "key hole number: " << keyHoleNumber << "\n";
		}

		const uint16_t doorLevel = door->getDoorLevel();
		if (doorLevel != 0) {
			stream << "door level: " << doorLevel << "\n";
		}

		const uint16_t questNumber = door->getQuestNumber();
		if (questNumber != 0) {
			stream << "quest number: " << questNumber << "\n";
		}

		const uint16_t questValue = door->getQuestValue();
		if (questValue != 0) {
	...
	if (key) {
		const uint16_t keyNumber = key->getKeyNumber();
		if (keyNumber != 0) {
...
void MapDrawer::DrawTile(TileLocation* location)
{
	...
	if (options.show_tooltips) {
		if (tile->creature && !tile->spawn && map_z == floor) {
			if (tile->creature->isNpc()) {
				tooltip << "NPC";
				MakeTooltip(draw_x, draw_y, tooltip.str(), 0, 255, 0);
			} else {
				tooltip << "x" << tile->creature->getAmount() << std::endl << tile->creature->getSpawnTime() << "s";
				MakeTooltip(draw_x, draw_y, tooltip.str(), 255, 0, 0);
			}
		}
	}
	tooltip.str("");
