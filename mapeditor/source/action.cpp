		...
		switch(c->type) {
			case CHANGE_TILE: {
				...
				if(oldtile) {
					...
					else if (newtile->getLocation()->getSpawnCount() == 0) {
						if (oldtile->creature && !oldtile->spawn) {
							if (newtile->creature) {
								editor.map.tvpSpawns.removeSpawn(oldtile);
								editor.map.tvpSpawns.addSpawn(newtile);
							}
							else {
								editor.map.tvpSpawns.removeSpawn(newtile);
							}
						}
						else if (newtile->creature && !newtile->spawn) {
							editor.map.tvpSpawns.addSpawn(newtile);
						}
					}
	...
	while(it != changes.rend()) {
		...
		switch(c->type) {
			case CHANGE_TILE: {
				...
				else if (oldtile->getLocation()->getSpawnCount() == 0) {
					if (oldtile->creature && !oldtile->spawn) {
						if (newtile->creature) {
							editor.map.tvpSpawns.removeSpawn(newtile);
							editor.map.tvpSpawns.addSpawn(oldtile);
						}
						else {
							editor.map.tvpSpawns.addSpawn(oldtile);
						}
