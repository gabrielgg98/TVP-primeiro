...
bool Item::readItemAttribute_OTBM(const IOMap& maphandle, OTBM_ItemAttribute attr, BinaryNode* stream)
{
	switch (attr) {
		...
		case OTBM_ATTR_KEYNUMBER: return stream->skip(2);
		case OTBM_ATTR_HOUSEDOORID: return stream->skip(1);
		case OTBM_ATTR_TELE_DEST: return stream->skip(5);
		case OTBM_ATTR_KEYHOLENUMBER: return stream->skip(2);
		case OTBM_ATTR_DOORLEVEL: return stream->skip(2);
		case OTBM_ATTR_DOORQUESTNUMBER: return stream->skip(2);
		case OTBM_ATTR_DOORQUESTVALUE: return stream->skip(2);
...
bool Door::readItemAttribute_OTBM(const IOMap& maphandle, OTBM_ItemAttribute attribute, BinaryNode* stream)
{
	if (OTBM_ATTR_HOUSEDOORID == attribute) {
		...
		doorId = id;
		return true;
	} else if (OTBM_ATTR_KEYHOLENUMBER == attribute) {
		uint16_t number = 0;
		if (!stream->getU16(number)) {
			return false;
		}
		keyHoleNumber = number;
		return true;
	} else if (OTBM_ATTR_DOORLEVEL == attribute) {
		uint16_t level = 0;
		...
		doorLevel = level;
		return true;
	} else if (OTBM_ATTR_DOORQUESTNUMBER == attribute) {
		uint16_t number = 0;
		if (!stream->getU16(number)) {
			return false;
		}
		questNumber = number;
		return true;
	} else if (OTBM_ATTR_DOORQUESTVALUE == attribute) {
		uint16_t value = 0;
		if (!stream->getU16(value)) {
			return false;
		}
		questValue = value;
		return true;
...
void Door::serializeItemAttributes_OTBM(const IOMap& maphandle, NodeFileWriteHandle& stream) const
{
	...
	if (keyHoleNumber > 0) {
		stream.addU8(OTBM_ATTR_KEYHOLENUMBER);
		stream.addU16(keyHoleNumber);
	}

	if (doorLevel > 0) {
		stream.addU8(OTBM_ATTR_DOORLEVEL);
		stream.addU16(doorLevel);
	}

	if (questNumber > 0) {
		stream.addU8(OTBM_ATTR_DOORQUESTNUMBER);
		stream.addU16(questNumber);
	}

	if (questValue > 0) {
		stream.addU8(OTBM_ATTR_DOORQUESTVALUE);
		stream.addU16(questValue);
	}
}
...
bool Key::readItemAttribute_OTBM(const IOMap& maphandle, OTBM_ItemAttribute attribute, BinaryNode* stream)
{
	if (OTBM_ATTR_KEYNUMBER == attribute) {
		uint16_t number = 0;
		if (!stream->getU16(number)) {
			return false;
		}
...
void Key::serializeItemAttributes_OTBM(const IOMap& maphandle, NodeFileWriteHandle& stream) const
{
	Item::serializeItemAttributes_OTBM(maphandle, stream);
	if (keyNumber > 0) {
		stream.addByte(OTBM_ATTR_KEYNUMBER);
		stream.addU16(keyNumber);
	}
}
...
bool IOMapOTBM::loadSpawns(Map& map, pugi::xml_document& doc)
{
	...
	for(pugi::xml_node spawnNode = node.first_child(); spawnNode; spawnNode = spawnNode.next_sibling()) {
		if (as_lower_str(spawnNode.name()) == "tvpspawn") {
			...
			int32_t radius = spawnNode.attribute("radius").as_int(); // tvp spawns support 0 radius

			Tile* tile = map.getTile(spawnPosition);
			if (tile && tile->creature) {
				warning("Duplicate tvp-spawn on position %d:%d:%d\n", tile->getX(), tile->getY(), tile->getZ());
				continue;
			}
			...
			bool isNpc = false;
			std::string name;
			if (spawnNode.attribute("npcname")) {
				name = spawnNode.attribute("npcname").as_string();
				isNpc = true;
			} else {
				if (!spawnNode.attribute("monstername")) {
					warning("Invalid TVP Spawn at %d:%d:%d (no creature specified)\n", tile->getX(), tile->getY(), tile->getZ());
					continue;
				}
				name = spawnNode.attribute("monstername").as_string();
			}

			int32_t spawntime = spawnNode.attribute("spawntime").as_int();
			if (spawntime == 0) {
				spawntime = g_settings.getInteger(Config::DEFAULT_SPAWNTIME);
			}

			int32_t amount = spawnNode.attribute("amount").as_int();
			if (amount == 0) {
				amount = 1;
			}

			Direction direction = NORTH;
			int dir = spawnNode.attribute("direction").as_int(-1);
			...
			map.tvpSpawns.addSpawn(creatureTile);
		...
		} else if (as_lower_str(spawnNode.name()) == "spawn") {
			Position spawnPosition;
			spawnPosition.x = spawnNode.attribute("centerx").as_int();
			spawnPosition.y = spawnNode.attribute("centery").as_int();
			spawnPosition.z = spawnNode.attribute("centerz").as_int();
...
bool IOMapOTBM::saveSpawns(Map& map, pugi::xml_document& doc)
{
	...
	// save TVP spawns
	for (const auto& spawnPosition : map.tvpSpawns) {
		Tile* tile = map.getTile(spawnPosition);
		if (tile == nullptr)
			continue;
		...
		pugi::xml_node spawnNode = spawnNodes.append_child("tvpspawn");
		...
		if (!creature->isSaved()) {
			if (creature->isNpc()) {
				spawnNode.append_attribute("npcname") = creature->getName().c_str();
			} else {
				spawnNode.append_attribute("monstername") = creature->getName().c_str();
			}
			spawnNode.append_attribute("spawntime") = creature->getSpawnTime();
			spawnNode.append_attribute("amount") = creature->getAmount();
			if (creature->getDirection() != NORTH) {
				spawnNode.append_attribute("direction") = creature->getDirection();
			}

			spawnNode.append_attribute("radius") = std::max<int32_t>(1, creature->getRadius());
