	...
	description_field(nullptr)
{
	...
	if(Container* container = dynamic_cast<Container*>(edit_item)) {
		...
		Key* key = dynamic_cast<Key*>(edit_item);
		...
		*/
		...
		if(door) {
			...
			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Key Hole Number")));
			keyholenumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->getKeyHoleNumber()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->getKeyHoleNumber());
			subsizer->Add(keyholenumber_field, wxSizerFlags(1).Expand());

			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Door Level")));
			doorlevel_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->getDoorLevel()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->getDoorLevel());
			subsizer->Add(doorlevel_field, wxSizerFlags(1).Expand());

			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Door Quest Number")));
			doorquestnumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->getQuestNumber()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->getQuestNumber());
			subsizer->Add(doorquestnumber_field, wxSizerFlags(1).Expand());

			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Door Quest Value")));
			doorquestvalue_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(door->getQuestValue()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, door->getQuestValue());
			subsizer->Add(doorquestvalue_field, wxSizerFlags(1).Expand());
		}


		if (key) {
			subsizer->Add(newd wxStaticText(this, wxID_ANY, wxT("Key Number")));
			keynumber_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(key->getKeyNumber()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, key->getKeyNumber());
			subsizer->Add(keynumber_field, wxSizerFlags(1).Expand());
		}
	...
	description_field(nullptr)
{
	...
	subsizer->Add(newd wxStaticText(this, wxID_ANY, "Monster Amount"));
	amount_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(edit_creature->getAmount()), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 1, 12, edit_creature->getAmount());
	// count_field->SetSelection(-1, -1);
	subsizer->Add(amount_field, wxSizerFlags(1).Expand());

	subsizer->Add(newd wxStaticText(this, wxID_ANY, "Monster/NPC Walk Radius"));
	radius_field = newd wxSpinCtrl(this, wxID_ANY, i2ws(edit_creature->getRadius()), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 1, 1000, edit_creature->getRadius());
	// count_field->SetSelection(-1, -1);
	subsizer->Add(radius_field, wxSizerFlags(1).Expand());
...
void OldPropertiesWindow::OnClickOK(wxCommandEvent& WXUNUSED(event))
{
	if(edit_item) {
		if(dynamic_cast<Container*>(edit_item)) {
			...
			int new_keyholenumber = 0;
			int new_doorlevel = 0;
			int new_questnumber = 0;
			int new_questvalue = 0;
			if (door) {
				new_door_id = door_id_field->GetValue();
				new_keyholenumber = keyholenumber_field->GetValue();
				new_doorlevel = doorlevel_field->GetValue();
				new_questnumber = doorquestnumber_field->GetValue();
				new_questvalue = doorquestvalue_field->GetValue();
			}

			int new_keynumber = 0;
			if (key) {
				new_keynumber = keynumber_field->GetValue();
			}
			...
			if (new_keyholenumber > 0xFFFF) {
				g_gui.PopupDialog(this, "Error", "Key Hole Number must be lower than 65535.", wxOK);
				return;
			}

			if (new_doorlevel > 0xFFFF) {
				g_gui.PopupDialog(this, "Error", "Door Level must be lower than 65535.", wxOK);
				return;
			}

			if (new_questnumber > 0xFFFF) {
				g_gui.PopupDialog(this, "Error", "Door Quest Number must be lower than 65535.", wxOK);
				return;
			}

			if (new_questvalue > 0xFFFF) {
				g_gui.PopupDialog(this, "Error", "Door Quest Value must be lower than 65535.", wxOK);
				return;
			}

			if (new_keynumber > 0xFFFF) {
				g_gui.PopupDialog(this, "Error", "Key Number must be lower than 65535.", wxOK);
				return;
			}
			...
			*/
			...
			if (door) {
				...
				door->setKeyHoleNumber(new_keyholenumber);
				door->setDoorLevel(new_doorlevel);
				door->setQuestNumber(new_questnumber);
				door->setQuestValue(new_questvalue);
			}
			if (key) {
				key->setKeyNumber(new_keynumber);
			}
			if (teleport) {
