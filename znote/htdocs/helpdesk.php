...
if ($view !== false) {
	...
	$account = mysql_select_single("SELECT id,email FROM accounts WHERE id = $session_user_id");
	if (!empty($_POST)) {
	...
	if (isset($_GET['success']) && empty($_GET['success'])) {
		...
		<form action="" method="post">
			<ul>
				<li>
					Account Name:<br>
					<input type="text" name="username" size="40" value="<?php echo $account['id']; ?>" disabled>
				</li>
				<li>
					Email:<br>
				...
				?>
				<li>
					<input type="hidden" name="username" value="<?php echo $account['id']; ?>">
