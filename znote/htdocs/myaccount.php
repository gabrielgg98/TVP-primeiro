...
if (isset($_GET['authenticate']) && $config['mailserver']['myaccount_verify_email']):
	...
	if (isset($_GET['u']) && isset($_GET['k'])) {
		...
		if ($user !== false) {
			...
			$user_name = ($config['ServerEngine'] !== 'OTHIRE') ? $user_data['id'] : $user_data['id'];
...
if ($render_page) {
	...
	<div id="myaccount">
		<h1>My account</h1>
		<p>Welcome to your account page, <?php if ($config['ServerEngine'] !== 'OTHIRE') echo $user_data['id']; else echo $user_data['id']; ?><br>
